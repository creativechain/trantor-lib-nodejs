...
trantor.sendTx = function(addr, amount, fee, cback) {
  ...
  trantor.getWallets(function (wallets) {
      ...
      if (!wallets) {
        throw 'No wallets created, please create a wallet first. `trantor.newWIF()`'
      }
...
trantor.sendTxOp = function(amount, fee, data, cback) {
  var network = bitcoin.networks.creativecoin;
  var keyPair = bitcoin.ECPair.makeRandom({
    network: network
  });
  ...
  var wallets = trantor.getWallets(function (wallets) {
    ...
    while (iTx < nTx) {
      let itx_ = iTx+0;
      // setTimeout(function() {
      var dataI = '-CREA-'+data.substr(itx_ * 1024, 1018);
      ...
      var wifsigns = new Object();
      // if (!hasDoneUnspent) {
      _.each(wallets, function(wif, address) {
        trantor.listunspent(address, (data) => {
          hasDoneUnspent = true;
      ...
      setTimeout(function () {
        ...
        setTimeout(function() {
          ...
          _.each(wifsigns, function(key, value) {
            tx.sign(parseFloat(key), wifsigns[key]);
          });
...
if (subcommand) {
  switch (subcommand) {
    case 'sendTx':
      trantor.sendTx(args[1], args[2] || 0.002, args[3] || 0.002, function (response) {
