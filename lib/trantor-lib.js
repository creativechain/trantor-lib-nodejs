...
client.on('error', function (error) {
  console.log("There has been an error with torrent", error);
})
  ...
  db.all('SELECT * FROM lastexplored ORDER BY date ASC LIMIT 0,1', (_, res) => {
    ...
    if (res[0] && res[0].blockhash) {
      https.call('GET', '/api/getblockcount', null, (blockcount) => {
        ...
        listsinceblock(res[0].blockhash, res[0].untilblock || null, cback);//add lastblock['block']
      })
    }
    else {
...
function listsinceblock(starthash, lastblock, cback) {
  db.serialize(function () {
    ...
    function listBlock(starthash) {
      let insetAddr = db.prepare("INSERT INTO addrtotx VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
      let insetWord = db.prepare("INSERT INTO wordToReference VALUES (?, ?, ?, ?)");
      let insertCtx = db.prepare("INSERT INTO contracttx  VALUES (?, ?, ?, ?, ?, ?)")
          ...
          function processBlockTx(i) {
            ...
            if (tx_id) {
              getDecodedTransaction(tx_id, decodedintx => {
                ...
                if (i < block.tx.length - 1) {
                  ...
                  if (block.previousblockhash && block.previousblockhash != lastblock && block.previousblockhash != starthash) {
                    console.log("End Exploring Block ["+block.height+"]");
                    // db.all('DELETE FROM lastexplored', _ => {});
                    db.run('INSERT INTO lastexplored (blockhash, untilblock, date) VALUES ("'+starthash+'", "'+lastblock+'", "'+blocktime+'")', (_, a) => {});
              ...
              if (block.previousblockhash && block.previousblockhash != lastblock && block.previousblockhash != starthash) {
                ...
                // db.all('DELETE FROM lastexplored', _ => {});
                db.run('INSERT INTO lastexplored (blockhash, untilblock, date) VALUES ("'+starthash+'", "'+lastblock+'", "'+blocktime+'")', (_, a) => {console.log("ASDASSD: ", _, a);});
                listBlock(block.previousblockhash)
              } else if (!block.previousblockhash || block.previousblockhash == lastblock) {
                console.log("End lastblock");
                isExploring = false;
        ...
        setTimeout(function() {
          ...
          trantor.pushTx(txBuilt.toHex(), function (response) {
            if (cback && typeof cback == 'function') {
              ...
              cback({error: 'ERROR: Maybe data is not valie JSON or fees and amount are incorrect'})
            }
          });
        }, 3000);
      }, 3000)
...
trantor.seedFile = function (filepath, cback) {
  // console.log("Path", filepath);
  console.log("Seeding file: ", filepath);
  // if (fs.existsSync(filepath)) {
  //   throw filepath+' - file does not exist, please add a valid file';
  // }
...
trantor.getTorrent = function (magnet_url, path = './torrents', cback) {
  let done = false;
  console.log("Downloading torrent: "+magnet_url);
  console.log("Please wait...");

  client.add(magnet_url, {path: path}, function (torrent) {
      // torrent.files.forEach(function (file) {
      //   console.log("Got torrent file: "+torrent.path+'/'+torrent.name);
      //   // client.seed(new Buffer(file), {}, function (torrent) {})
      // })
      cback(torrent);
    })
}
...
if (subcommand) {
  switch (subcommand) {
    ...
    case 'seedFile':
      if (!args[1]) {
        throw 'seedFile requires 1 arguments <filepath>'
      }
      trantor.seedFile(args[1], function (torrent) {
        // console.log("Data: ", response);
        console.log("TORRENT: \n", torrent);
        console.log("\nPress [ctrl/cmd + c] to stop seeding");
      })
      break;
    case 'getTorrent':
      if (!args[1]) {
        throw 'getTorrent requires 1 arguments <magnet_url>'
      }
      return trantor.getTorrent(args[1], args[2] || '/torrents', function (response) {
        console.log("Data: ", response);
        return;
      })
    ...
    case 'getWallets':
      ...
      break;
    case 'explore':
      console.log(creaExplore(function() {
